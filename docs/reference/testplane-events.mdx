...
После этого testplane читает все тесты с файловой системы в мастер-процессе. Посылая для каждого файла, перед тем как его прочитать, событие [BEFORE_FILE_READ](#before_file_read) и после прочтения &mdash; событие [AFTER_FILE_READ](#after_file_read).
...
Далее воркеры получают от мастер-процесса информацию о конкретных тестах, которые они должны запустить. Так как тесты хранятся в файлах, то воркеры читают конкретно те файлы, в которых находятся требуемые тесты. И перед чтением каждого такого файла, в каждом воркере триггерится событие [BEFORE_FILE_READ](#before_file_read), а после прочтения &mdash; событие [AFTER_FILE_READ](#after_file_read).
...
Далее всё будет зависеть от того, с каким результатом завершился прогон теста. Если тест прошел успешно, то testplane стриггерит событие [TEST_PASS](#test_pass). Если тест упал &mdash; [TEST_FAIL](#test_fail). Если тест упал, но его следует запустить повторно (см. настройки [retry][browsers-retry] и [shouldRetry][browsers-should-retry] в конфиге testplane), то вместо события [TEST_FAIL](#test_fail) будет отправлено событие [RETRY](#retry).

Если тест не нужно запускать повторно, и результат &mdash; окончательный, то testplane триггерит события [TEST_END](#test_end) и [SUITE_END](#suite_end), если речь идет о завершении выполнения describe-блока.
...
Например, код ниже показывает, как можно перехватывать событие [TEST_FAIL](#test_fail) и изменять его на событие [TEST_PENDING](#test_pending) &mdash; то есть автоматически отключать падающие тесты, не давая им уронить общий прогон тестов:
...
<Admonition type="info" title="Что такое dev-сервер?">
    Dev-сервер &mdash; это [express][express]-like приложение, которое позволяет разрабатывать
    фронтенд проекта.
</Admonition>
...
-   `name` &mdash; имя хелпера (или иначе &mdash; контроллера);
-   `methods` &mdash; объект-словарь, ключи которого задают названия методов соответствующих хелперов, а значения ключей определяют их реализацию. Каждый метод будет вызван на соответствующем тесте или наборе тестов _(suite)_.
...
-   первый аргумент &mdash; инстанс `WebdriverIO`;
-   второй аргумент &mdash; объект вида `{ browserId, sessionId }`, где _browserId_ &mdash; это имя браузера, а _sessionId_ &mdash; идентификатор сессии браузера.
...
-   первый аргумент &mdash; инстанс `WebdriverIO`;
-   второй аргумент &mdash; объект вида `{ browserId, versionId }`, где _browserId_ &mdash; это имя браузера, а _browserVersion_ &mdash; версия браузера.
...
Запуск dev-сервера &mdash; опционален: для этого плагин добавляет специальную опцию `--dev-server` к testplane, позволяя разработчику указывать при запуске testplane, нужно ли поднимать dev-сервер.
...
Рассмотрим схематичную реализацию профилирования прогона тестов. При каждом запуске теста мы будем засекать время его запуска, а при завершении &mdash; время завершения. Всю информацию мы будем сохранять в стрим, который по завершению раннера будет закрываться.
