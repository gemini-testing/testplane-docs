...
[WebDriver][webdriver] &mdash; это стандартизованный REST API протокол. Разработчики браузеров поддерживают данный протокол в своих драйверах &mdash; [chromedriver][chromedriver], [geckodriver][geckodriver], и т. п., &mdash; которые являются посредниками (прокси) между клиентом, отправляющим запросы, и самими браузерами. Такой посредник нужен, так как браузеры написаны совершенно по-разному и общение между драйвером и браузером никак не стандартизированно.
...
[Chrome DevTools Protocol (CDP)][cdp] &mdash; это по сути [JSON RPC][json-rpc], реализованный через [вебсокеты][websockets].
...
-   в IDE (например, в _VSCode)_ &mdash; для аналогичных целей;
-   в различных инструментах автоматизации тестирования: [puppeteer][puppeteer], [cypress][cypress], и пр.;
-   при общении между [chromedriver][chromedriver] и браузером Chrome (на картинке выше &mdash; `browser protocol`).
