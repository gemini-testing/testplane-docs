...
<Admonition type="info">
    ...
    in guide [How to launch Testplane in the local browser](/docs/basic-guides/browsers-overview).
</Admonition>
...
Move the cursor to point _(0, 0)_ in _body_ coordinates before each test run. Default: `false`. It may be needed in cases where the default cursor position affects test execution. For example, when a tooltip _(hint)_ pops up because of the cursor, which "spoils" the screenshot being taken.

<Admonition type="warning">
    Default value was changed from `true` to `false` in testplane@9.0.0.
</Admonition>
