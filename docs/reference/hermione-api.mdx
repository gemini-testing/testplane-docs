...
Первым аргументом функция принимает событие, которое нужно перехватывать, а вторым &mdash; обработчик события.
...
Возвращает `true`, если прогон тестов завершился успешно, и `false` &mdash; если не успешно.
