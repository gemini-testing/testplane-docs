...
Где `<browser-id>` &mdash; это имя браузера, которое используется для его идентификации.
...
Эндпойнт websocket-соединения для подключения к браузеру через [Chrome DevTools Protocol (CDP)][how-to-use-cdp]. Например, вы указываете `browserWSEndpoint: "ws://YOUR_HOST/devtools"`, к которому в конце будет добавлен идентификатор сеанса браузера: `ws://YOUR_HOST/devtools/12345`, где `12345` &mdash; это идентификатор сеанса.
...
<tr><td>[saveHistory](#save_history)</td><td>Boolean</td><td>true</td><td>Сохранять историю всех выполненных команд. _В testplane@7 заменена на `saveHistoryMode`_.</td></tr>
<tr><td>[saveHistoryMode](#save_history_mode)</td><td>"all" или "none" или "onlyFailed"</td><td>"all"</td><td>Сохранять историю всех выполненных команд. _Доступна с testplane@7_.</td></tr>
...
-   `viewport` &mdash; снять скриншот текущего вьюпорта.
-   `fullpage` &mdash; снять скриншот всей страницы браузера.
...
<Admonition type="warning">
    В testplane@7 заменена на [saveHistoryMode](#save_history_mode).
</Admonition>
...
История доступна из следующих событий &mdash; `TEST_END`, `TEST_PASS`, `TEST_FAIL` &mdash; через `payload`:
...
<Admonition type="warning">Доступна с testplane@7.</Admonition>

Сохранять историю всех выполненных команд. По умолчанию: `all`.

Available options:

-   `all` &mdash; история включена.
-   `none` &mdash; история отключена.
-   `onlyFailed` &mdash; история сохраняется только для упавших тестов.
...
Папка для сохранения эталонных скриншотов командой `assertView`. По умолчанию это папка `testplane/screens` относительно рабочей папки: `process.cwd()`. Значением данной опции может быть также функция, которой передается один аргумент &mdash; инстанс теста, внутри которого была вызвана команда `assertView`, например:
...
Позволяет перехватывать и преобразовывать [http-ответ][got-response], полученный от WebDriver. По умолчанию: `null`. Если передается функция, то первым аргументом ей будет передан объект `Response`, а вторым &mdash; `RequestOptions`. В ответ функция должна будет вернуть модифицированный `Response`.
