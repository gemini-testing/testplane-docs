...
-   `a*=text` — text match inside an `<a>` tag
-   `div.=text` — case-insensitive text match inside a `<div>` tag
...
To use this type of selector, you need to [install Testing Library](../../guides/how-to-add-testing-library).
...
describe("Finding an input field using the getByLabelText method", () => {
    it("Find and use the search field", async ({ browser }) => {
        await browser.url("https://testplane.io/docs/html-reporter/overview/");
