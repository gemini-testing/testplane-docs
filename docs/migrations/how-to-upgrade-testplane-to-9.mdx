...
If you notice some undesired changes in your screenshots, check the [Common issues](#common_issues) section below or, if you can't find a solution, let us know in [github issues](https://github.com/gemini-testing/testplane/issues).
...
Alternatively, you can use triple-slash directive: `/// <reference types="@testplane/globals" />`.
...
1. Compute "safe area", that is free of sticky, fixed, and other interfering elements that are not part of the target element. Areas with border radiuses are also considered unsafe.
2. At all times during the capture, the engine will try to keep the target element within the safe area, including at the very beginning.
...
The other reason might be that you are using a custom command on top of `assertView`, that draws transparent overlay on top of the page and screenshots that. In that case, we recommend to use bare `assertView` instead.
