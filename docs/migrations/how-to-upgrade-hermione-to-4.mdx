...
Таким образом, причин для радикального апгрейда &mdash; сразу на 3 мажора вверх &mdash; накопилось достаточно.
...
<Admonition type="warning">
    ...
    младше _4.9.0_, то к браузеру в тестах нужно обращаться как и раньше &mdash; через _this_,
    например: _await this.browser.getText('.selector')_.
</Admonition>
...
Бонус для тех, кто уже перешел на React &mdash; теперь в тестах можно использовать команды [`browser.react$`][browser-react-dollar] и [`browser.react$$`][browser-react-dollar-dollar] для поиска на странице конкретных react-компонентов с определенными состояниями. Аналогичные команды есть и для элементов &mdash; [`element.react$`][element-react-dollar] и [`element.react$$`][element-react-dollar-dollar].
...
Раньше для того, чтобы запустить тесты локально на своем браузере, нужно было запустить `selenium-standalone` и указать hermione магический `gridUrl`, чтобы все заработало. Сейчас же &mdash; достаточно в конфиге указать браузеру опцию `automationProtocol` со значением `devtools`:
...
Мы обновились сразу на 3 мажора _webdriverio_, поэтому просто обновить версию hermione в `package.json` не получится. Основные проблемы при переезде &mdash; это отсутствующий _chaining_ в тестах и устаревшие команды тестов. Чтобы вам было проще разобраться с обеими проблемами, мы написали для вас следующую инструкцию.
