...
| `sessionsPerBrowser`| min(слоты_грида_для_браузера, доступная_RAM / RAM_одного_браузера) | Реальное ограничение — ресурсы грида (слоты и RAM)                                                          |
| `testsPerSession`   | min(тесты_до_деградации_браузера, тесты_за_время_жизни_сессии_грида) | Сессию нельзя переиспользовать бесконечно — браузер деградирует. Если грид принудительно закрывает сессию по времени, нужно вписаться в этот лимит. Параметр не влияет на параллельность, но влияет на стабильность прогона. |
