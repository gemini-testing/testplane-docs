...
Когда воркер получает тесты для Chrome, он открывает до 5 окон браузера одновременно. Каждое окно — это отдельная независимая сессия со своим `sessionId`:
...
Логика та же, но одновременно работают только 2 окна Firefox. Это ограничение задано через `sessionsPerBrowser`:
...
#### Как объединить отчеты чанков

Чтобы объединить несколько отчетов в один, используйте команду `merge-reports`
