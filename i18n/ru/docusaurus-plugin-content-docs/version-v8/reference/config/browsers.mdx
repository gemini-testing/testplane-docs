...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>`string`</td>
            <td>`"webdriver"`</td>
            <td>Протокол общения с браузером. См. [WebDriver vs CDP](../../webdriver-vs-cdp).</td>
        </tr>
        <tr>
...
<Admonition type="info">
    ...
    браузере](/docs/v8/basic-guides/browsers-overview).
</Admonition>
...
Перемещать курсор в точку _(0, 0)_ в координатах _body_ перед каждым запуском теста. По умолчанию: `false`. Может потребоваться в тех случаях, когда положение курсора по умолчанию влияет на выполнение тестов. Например, когда из-за курсора всплывает какая-нибудь подсказка _(hint)_, которая «портит» снимаемый скриншот.

<Admonition type="warning">
    Значение по умолчанию было изменено с `true` на `false` в testplane@9.0.0.
</Admonition>
