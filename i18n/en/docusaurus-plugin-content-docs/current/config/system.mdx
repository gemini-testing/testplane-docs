...
* `viteConfig` - ability to specify own [Vite configuration][vite-config]. You can pass relative path to the config file as `string`, object with [UserConfig][vite-user-config] type or function with `(env: ConfigEnv) => UserConfig | Promise<UserConfig>` type.
