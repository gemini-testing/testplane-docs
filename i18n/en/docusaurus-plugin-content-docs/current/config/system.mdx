...
-   `viteConfig` — custom [Vite config](https://vitejs.dev/config/). You can specify a string — path to the configuration file, an object — [UserConfig](https://github.com/vitejs/vite/blob/v5.1.6/packages/vite/src/node/config.ts#L127-L282) or a function — with the type `(env: ConfigEnv) => UserConfig | Promise<UserConfig>`.
